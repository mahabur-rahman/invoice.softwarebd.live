...
const InvoiceItemRow = ({ index, item, onChange, onRemove }: ItemRowProps) => (
  <div className="grid grid-cols-12 gap-2 items-center mb-2">
    ...
    <input
      ...
      min={1}
      value={item.qty}
      onChange={(e) => onChange(index, { ...item, qty: Number(e.target.value) })}
    ...
    <input
      ...
      min={0}
      value={item.price}
      onChange={(e) => onChange(index, { ...item, price: Number(e.target.value) })}
      className="col-span-2 p-2 border border-gray-300 rounded-lg text-center focus:ring-2 focus:ring-blue-500 focus:outline-none"
    />
    <div className="col-span-2 text-right font-semibold text-gray-700">${(item.qty * item.price).toFixed(2)}</div>
